...
const Post = ({
  ...
  const [liked, setLiked] = useState(
    post.likedBy.includes(localStorage.getItem("email"))
  );
    ...
    <Card className="card" sx={{ borderRadius: 2 }}>
      <Box className="centered" sx={{ p: 1 }}>
        <Box className="centered" sx={{ justifyContent: "flex-start" }}>
          <Avatar src={profilePicture} className="avatar2">
            {name[0]}
          </Avatar>
          <Box sx={{ ml: 1, cursor: "pointer" }} onClick={() => goToProfile()}>
            <Typography
              variant="body1"
              sx={{ p: 0, m: 0, color: "black", fontWeight: "800" }}
            >
              {name}
            </Typography>
      ...
      <Box className="postBottom">
        <Box className="centered">
          <Button size="small" color="primary">
            <SentimentSatisfiedAltIcon fontSize="small" color="secondary" />
        ...
        <CardActions className="cardActions">
          <Button
            ...
            onClick={() => likePost(post._id,post.likedBy.length)}
          >
            <ThumbUpIcon
          ...
          <Button size="small" color="primary">
            <ChatBubbleIcon fontSize="small" /> comments
          </Button>
          <Button
            size="small"
            color="primary"
